...
RUN mkdir -p /home/app/.local/share /data /sockets && \
...
COPY --from=publish --chown=1654:1654 /sockets /sockets
COPY --from=publish --chown=1654:1654 /app/publish .
ENTRYPOINT ["dotnet", "TerrariaServerModded.dll", "--data-path", "/data", "--socket-dir", "/sockets"]
